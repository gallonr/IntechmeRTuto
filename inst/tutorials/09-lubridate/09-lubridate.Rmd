...
subtitle: "Maîtriser les données temporelles"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
evenements <- tibble(
  nom = c("Réunion", "Formation", "Conférence", "Atelier", "Séminaire"),
...
Travailler avec des dates et des heures en R peut être compliqué : formats différents, fuseaux horaires, calculs de durées... Le package `lubridate` simplifie considérablement ces tâches et rend le code beaucoup plus lisible !

**Objectifs d'apprentissage :**

- ✅ **Parser** des dates avec `ymd()`, `dmy()`, `mdy()`
- ✅ **Extraire** des composants avec `year()`, `month()`, `day()`, etc.
- ✅ **Calculer** des durées et des intervalles
- ✅ **Manipuler** des dates (ajouter/soustraire des périodes)
- ✅ **Arrondir** des dates avec `floor_date()`, `ceiling_date()`
- ✅ Travailler avec des **heures** et des **timestamps**
...
En R, les dates sont un type spécial stocké comme le nombre de jours depuis le 1er janvier 1970.
...
- `ymd()` : année-mois-jour (format ISO, recommandé)
- `dmy()` : jour-mois-année (format français/européen)
...
💡 Ces fonctions sont **très flexibles** avec les séparateurs !
...
```{r ex1-parse-hint}
# Le format est jour/mois/année → utilisez dmy()
```
...
month(ma_date, label = TRUE)              # Abréviation
...
Extrayez le mois et l'année de chaque commande :
...
La soustraction de dates donne une durée :

```{r demo-duree, exercise=TRUE}
debut <- ymd("2024-01-01")
fin <- ymd("2024-01-15")

fin - debut  # Différence en jours
```

### Exercice 3 : Délai de livraison

Calculez le délai entre la commande et la livraison :
...
quiz(
  ...
  question("Quelle fonction parse une date au format jour/mois/année ?",
  ...
  question("Comment extraire le mois d'une date sous forme de texte (ex: 'Jan', 'Fév') ?",
  ...
  question("La soustraction de deux dates donne :",
    answer("Une durée en jours", correct = TRUE),
    answer("Une durée en heures"),
    answer("Une erreur"),
    answer("Un nombre"),
    allow_retry = TRUE
  ),
...
## Section 5 : Ajouter et soustraire des périodes

### Les fonctions de période
...
### Exercice 4 : Date d'échéance

Calculez la date d'échéance (30 jours après la commande) :
...
Ces fonctions arrondissent les dates à une unité donnée :

```{r demo-round, exercise=TRUE}
date_precise <- ymd_hms("2024-03-15 14:35:42")

# Arrondir au début
floor_date(date_precise, "day")    # Début du jour
floor_date(date_precise, "month")  # Début du mois
floor_date(date_precise, "year")   # Début de l'année
...
### Exercice 6 : Durée des événements

Calculez la durée de chaque événement en heures :
...
# 2. Calculez le délai de livraison en jours
# 3. Extrayez le mois de commande (avec label)
# 4. Créez une colonne "trimestre" (Q1, Q2, Q3, Q4)
# 5. Calculez les statistiques par trimestre :
#    - Nombre de commandes
#    - Délai moyen
#    - Montant total
# 6. Affichez le résultat trié par trimestre
...
- ✅ **Parser** des dates avec `ymd()`, `dmy()`, `mdy()` et variantes
- ✅ **Extraire** des composants avec `year()`, `month()`, `day()`, `hour()`, etc.
- ✅ **Calculer** des durées entre dates
- ✅ **Ajouter/soustraire** des périodes avec `days()`, `months()`, `years()`
- ✅ **Arrondir** des dates avec `floor_date()`, `ceiling_date()`
- ✅ Travailler avec des **timestamps** et des **intervalles**
...
# PARSER DES DATES
ymd("2024-01-15")          # Année-mois-jour
dmy("15/01/2024")          # Jour-mois-année
mdy("01/15/2024")          # Mois-jour-année
ymd_hms("2024-01-15 14:30:00")  # Avec heure

# EXTRAIRE DES COMPOSANTS
year(date)                 # Année
month(date)                # Mois (numérique)
month(date, label = TRUE)  # Mois (texte)
day(date)                  # Jour
wday(date)                 # Jour de la semaine
yday(date)                 # Jour de l'année
hour(datetime)             # Heure
...
as.numeric(diff, units = "hours")  # Convertir l'unité
...
# ARRONDIR
floor_date(date, "month")    # Début du mois
ceiling_date(date, "month")  # Fin du mois
round_date(date, "day")      # Arrondir au jour

# INTERVALLES
interval(debut, fin)         # Créer un intervalle
date %within% intervalle     # Tester appartenance
time_length(intervalle, "days")  # Durée

# CRÉER DES DATES
today()                      # Date du jour
now()                        # Date-heure actuelle
make_date(2024, 1, 15)      # Créer une date
make_datetime(2024, 1, 15, 14, 30)  # Créer date-heure
```
