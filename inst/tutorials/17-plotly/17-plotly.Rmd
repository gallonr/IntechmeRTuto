...
- ✅ Convertir ggplot en graphique interactif avec `ggplotly()`
- ✅ Créer des graphiques avec **plotly natif**
- ✅ Personnaliser les **tooltips** (infobulles)
- ✅ Utiliser les **interactions** : zoom, pan, hover
...
- 📍 **Pan** : déplacer la vue
- 🖱️ **Hover** : afficher les valeurs au survol
- 📷 **Export** : télécharger l'image
- 🔄 **Reset** : revenir à la vue initiale

### Exercice 1 : Premier ggplotly()

Créez un graphique interactif de la masse vs nageoires :
...
### Exercice 2 : Tooltips personnalisés

Créez des tooltips affichant espèce, sexe et masse :
...
plot_ly(data = penguins,
        ...
        color = ~species,
        type = "box") |>
  layout(title = "Masse par espèce")
```

### Exercice 3 : Graphique en barres interactif

Créez un graphique en barres du nombre de manchots par île :
...
plot_ly(data = comptage,
        ...
        y = ~_____,
        type = "_____") |>
  layout(title = "Nombre de manchots par île")
```
...
# Graphique avec plusieurs espèces
...
### Exercice 4 : 3D personnalisé

Créez un graphique 3D avec nageoires, bec et masse :
...
plot_ly(data = penguins,
  ...
  layout(
    ...
    title = "Relations morphométriques 3D"
  )
```

---

## Section 6 : Séries temporelles
...
### Exercice 5 : Série temporelle

Créez un graphique de l'évolution du nombre de manchots par année et espèce :
...
# Calculer moyenne de masse par espèce et île
...
plot_ly(
  ...
  layout(title = "Masse moyenne par espèce et île",
         xaxis = list(title = "Île"),
         yaxis = list(title = "Espèce"))
```
...
4. Un titre et des axes bien formatés

```{r defi-final, exercise=TRUE, exercise.lines=15}
# Préparer les données avec tooltips
...
plot_ly(data = penguins_prep,
  ...
  layout(
    title = list(
      text = "<b>Analyse morphométrique 3D des manchots</b><br><i>Données Palmer LTER</i>",
      font = list(size = 16)
    ),
    scene = list(
