...
# Créer les données des manchots
penguins_locations <- tibble(
  island = c("Torgersen", "Biscoe", "Dream"),
  longitude = c(-64.08, -65.43, -64.73),
  latitude = c(-64.77, -65.50, -64.23),
...
### Exercice 4 : Carte multi-couches avec fond réel

Créez une carte avec :
1. Fond de la France (vrai fond de carte)
2. Points de villes
3. Étiquettes

```{r ex4-layers, exercise=TRUE}
# Charger le fond de carte France
france <- ne_countries(country = "france", scale = "medium", returnclass = "sf")

# Villes côtières
villes <- tibble(
  nom = c("Brest", "Nantes", "Bordeaux", "Marseille"),
  lon = c(-4.49, -1.55, -0.57, 5.37),
  lat = c(48.39, 47.22, 44.84, 43.30),
  population = c(140000, 320000, 260000, 870000)
) |> st_as_sf(coords = c("lon", "lat"), crs = 4326)

# Carte
ggplot() +
  geom_sf(data = _____, fill = "lightgreen", alpha = 0.3) +
  geom_sf(data = _____, aes(size = _____), color = "darkred", alpha = 0.7) +
  geom_sf_text(data = _____, aes(label = _____), nudge_y = 0.3, fontface = "bold")
```
...
# Carte complète
ggplot() +
  geom_sf(data = france, fill = "lightgreen", alpha = 0.3, color = "darkgreen", linewidth = 0.5) +
  geom_sf(data = villes, aes(size = population), color = "darkred", alpha = 0.7) +
  geom_sf_text(data = villes, aes(label = nom), nudge_y = 0.5, fontface = "bold", size = 3.5) +
  scale_size_continuous(range = c(3, 10), name = "Population") +
  labs(title = "Principales villes côtières de France") +
  theme_minimal() +
  coord_sf(xlim = c(-5, 8), ylim = c(42, 51))
```

---

## Section 6 : Fonds de cartes avec rnaturalearth

### Charger des fonds de cartes mondiaux

```{r demo-naturalearth, exercise=TRUE}
# Charger rnaturalearth
library(rnaturalearth)
library(rnaturalearthdata)
...
ggplot(europe) +
  ...
  labs(title = "Carte d'Europe") +
  theme_minimal()
```

### Carte de France avec données

```{r demo-france-map, exercise=TRUE}
# Fond de carte France
france <- ne_countries(country = "france", scale = "medium", returnclass = "sf")

# Stations marines
stations <- tibble(
  nom = c("Roscoff", "Concarneau", "La Rochelle", "Sète"),
  lon = c(-3.98, -3.92, -1.15, 3.70),
  lat = c(48.73, 47.87, 46.16, 43.40),
  temperature = c(12.5, 13.2, 14.8, 16.5)
) |> st_as_sf(coords = c("lon", "lat"), crs = 4326)

# Carte combinée
ggplot() +
  geom_sf(data = france, fill = "wheat", color = "gray40") +
  geom_sf(data = stations, aes(color = temperature), size = 5) +
  scale_color_gradient(low = "blue", high = "red", name = "Temp (°C)") +
  geom_sf_text(data = stations, aes(label = nom), 
               nudge_y = 0.4, fontface = "bold", size = 3) +
  labs(title = "Stations marines en France") +
  theme_minimal() +
  coord_sf(xlim = c(-5, 8), ylim = c(42, 51))
```
...
```{r defi-final, exercise=TRUE, exercise.lines=30}
# 1. Fond de carte Europe de l'Ouest
europe <- ne_countries(scale = "medium", continent = "europe", returnclass = "sf")

# 2. Zones économiques exclusives (ZEE) simulées
zee_france <- tibble(
  pays = "France",
  zone = "Atlantique",
  richesse_halieutique = 85,
...
# 3. Ports de pêche
ports <- tibble(
  nom = c("Boulogne", "Concarneau", "Lorient", "La Rochelle", "Hendaye"),
  lon = c(1.61, -3.92, -3.37, -1.15, -1.77),
  lat = c(50.73, 47.87, 47.75, 46.16, 43.36),
  tonnage = c(35000, 28000, 22000, 18000, 12000)
) |> st_as_sf(coords = c("lon", "lat"), crs = 4326)

# 4. Créer la carte complète avec fond réel
ggplot() +
  geom_sf(data = _____, fill = _____, alpha = _____) +
  geom_sf(data = _____, aes(fill = _____), alpha = _____) +
  geom_sf(data = _____, aes(size = _____), color = _____) +
  _____  # Ajoutez échelles, labels, thème
```
...
# 4. Reprojection en Lambert-93 pour la France
europe_proj <- st_transform(europe, 2154)
zee_proj <- st_transform(zee_france, 2154)
ports_proj <- st_transform(ports, 2154)

# 5. Carte finale publication-ready
ggplot() +
  # Fond : pays européens
  geom_sf(data = europe_proj, fill = "gray95", color = "gray60", linewidth = 0.3) +
  # ZEE française colorée
  geom_sf(data = zee_proj, aes(fill = richesse_halieutique), 
          alpha = 0.4, color = "steelblue", linewidth = 1, linetype = "dashed") +
  scale_fill_gradient(
    low = "lightyellow", high = "darkgreen",
    name = "Richesse\nhalieutique"
  ) +
  # Ports avec taille proportionnelle au tonnage
  geom_sf(data = ports_proj, aes(size = tonnage), 
          color = "darkred", fill = "orange", shape = 21, stroke = 1.5, alpha = 0.8) +
  scale_size_continuous(
    range = c(4, 12),
    name = "Tonnage\n(tonnes/an)",
    labels = scales::comma
  ) +
  # Étiquettes des ports
  geom_sf_text(data = ports_proj, aes(label = nom),
               nudge_x = 80000, nudge_y = 30000, 
               fontface = "bold", size = 3, color = "gray20") +
  # Titre et annotations
  labs(
    title = "Cartographie halieutique - Façade Atlantique française",
    subtitle = "Principaux ports de pêche et zones d'exploitation",
    caption = "Source: Données simulées | @ Cnam-Intechmer - 2025 | Projection: Lambert-93 (EPSG:2154)"
  ) +
  # Limites géographiques (France Atlantique)
  coord_sf(xlim = c(-400000, 1200000), ylim = c(6100000, 7200000)) +
  # Thème professionnel
  theme_minimal() +
  theme(
    plot.title = element_text(size = 15, face = "bold", hjust = 0.5, color = "gray10"),
    ...
    panel.grid = element_line(color = "gray90", linetype = "dotted", linewidth = 0.3),
...
# FONDS DE CARTES (rnaturalearth)
ne_countries(scale = "medium", returnclass = "sf")           # Monde
ne_countries(country = "france", returnclass = "sf")         # Un pays
ne_countries(continent = "europe", returnclass = "sf")       # Un continent
ne_states(country = "france", returnclass = "sf")            # Régions/États

# LIRE DES FICHIERS
st_read("fichier.shp")               # Shapefile
st_read("fichier.geojson")           # GeoJSON
st_read("fichier.gpkg")              # GeoPackage
...
coord_sf(xlim = c(-5, 10), ylim = c(40, 52))  # Limites
...
# 2154  : Lambert-93 (France métropolitaine)
# 3031  : Antarctic Polar Stereographic
# 3857  : Web Mercator (cartes web)
```
