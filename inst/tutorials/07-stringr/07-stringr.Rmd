...
subtitle: "Maîtriser les chaînes de caractères"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
description: "Détecter, extraire, remplacer et transformer du texte avec stringr"
---
...
produits <- tibble(
  id = 1:6,
  nom = c("Ordinateur Portable HP", "Souris Logitech", "Clavier Mécanique",
          "Écran 24 pouces", "USB 3.0 64GB", "Casque Bluetooth"),
  prix_txt = c("899.99€", "12.50€", "145.00€", "299.99€", "25.50€", "89.90€"),
...
Les données textuelles sont partout : noms, adresses, descriptions de produits, commentaires... Mais elles sont souvent désordonnées, avec des espaces en trop, des majuscules/minuscules incohérentes, ou des formats variés. Le package `stringr` vous donne les outils pour nettoyer et transformer efficacement vos chaînes de caractères.

**Objectifs d'apprentissage :**

- ✅ Détecter des motifs avec `str_detect()` et `str_count()`
- ✅ Extraire du texte avec `str_extract()` et `str_sub()`
- ✅ Remplacer du texte avec `str_replace()` et `str_remove()`
- ✅ Transformer la casse avec `str_to_lower()`, `str_to_upper()`, `str_to_title()`
- ✅ Nettoyer les espaces avec `str_trim()` et `str_squish()`
- ✅ Découper et combiner avec `str_split()` et `str_c()`
...
## Section 1 : Détecter des motifs

### La fonction str_detect()

`str_detect()` teste si un motif est présent dans une chaîne. Retourne `TRUE` ou `FALSE`.

```{r demo-detect, exercise=TRUE}
# Vérifier si "Ordinateur" est dans le nom
produits |>
...
💡 Le symbole `|` signifie "OU" en expressions régulières.

### Exercice 1 : Détecter un motif
...
💡 `[aeiou]` est une **classe de caractères** : correspond à n'importe quelle voyelle.
...
`str_extract()` extrait la première occurrence d'un motif.
...
💡 `[0-9.]+` signifie : un ou plusieurs (`+`) chiffres ou points.
...
Extrayez le code numérique de la référence (les 3 derniers chiffres) :
...
```{r ex2-extract-hint}
# Le pattern pour 3 chiffres à la fin : "[0-9]{3}$"
# {3} = exactement 3
# $ = fin de chaîne
...
`str_sub()` extrait par position (début et fin).
...
`str_replace()` remplace la **première** occurrence d'un motif.

```{r demo-replace, exercise=TRUE}
# Remplacer "€" par " euros"
produits |>
  mutate(prix_txt = str_replace(prix_txt, "€", " euros"))
```
...
# Uniformiser les téléphones : retirer tous les séparateurs
clients |>
  mutate(tel_propre = str_replace_all(telephone, "[ .+-]", ""))
```

💡 `[ .+-]` = classe de caractères : espace, point, plus ou tiret.

### Exercice 3 : Remplacer du texte

Remplacez tous les points par des tirets dans les téléphones :
...
# Note : le point doit être échappé avec \\ car c'est un caractère spécial
```
...
`str_remove()` supprime un motif (équivalent à remplacer par "").

```{r demo-remove, exercise=TRUE}
# Retirer "€" des prix
produits |>
  mutate(prix_nombre = str_remove(prix_txt, "€"))
```
...
## Section 4 : Quiz - Détection et remplacement

```{r quiz-string-1}
quiz(
  caption = "Testez vos connaissances",
  
  question("Quelle fonction teste si un motif est présent dans une chaîne ?",
  ...
  question("Quelle est la différence entre str_replace() et str_replace_all() ?",
    answer("str_replace() remplace seulement la première occurrence", correct = TRUE),
    answer("str_replace() est plus rapide"),
    answer("str_replace() ne fonctionne qu'avec des regex"),
    answer("Aucune différence"),
    allow_retry = TRUE
  ),
  
  question("Comment extraire tous les nombres d'une chaîne ?",
...
## Section 7 : Longueur et découpage
...
`str_split()` découpe une chaîne selon un séparateur.

```{r demo-split, exercise=TRUE}
# Découper une référence
...
`str_c()` combine (concatène) des chaînes.
...
Avec `sep` pour ajouter un séparateur :
...
## Section 8 : Expressions régulières courantes

### Patterns utiles

| Pattern | Signification | Exemple |
|---------|---------------|---------|
| `^texte` | Début de chaîne | `^ORD` trouve "ORD-2024" |
| `texte$` | Fin de chaîne | `001$` trouve "ORD-2024-001" |
| `[abc]` | Un de ces caractères | `[aeiou]` trouve voyelles |
| `[^abc]` | Tout sauf ces caractères | `[^0-9]` trouve non-chiffres |
| `.` | N'importe quel caractère | `a.b` trouve "aXb" |
...
## Section 9 : Défi final

### Mission : Nettoyage complet de données clients

Nettoyez la base de données clients :
...
# 3. Vérifiez que les emails contiennent @
# 4. Uniformisez les téléphones : format sans séparateurs (10 chiffres)
...
```{r defi-final-hint-2}
# Ajoutez la vérification email et nettoyage téléphone
clients |>
  mutate(
    nom_propre = nom |> str_trim() |> str_to_title(),
    email_propre = str_to_lower(email),
...
clients |>
  mutate(
    ...
    email_propre = str_to_lower(email),
    # 3. Vérification emails
    email_valide = str_detect(email_propre, "@.+\\..+"),
    # 4. Uniformisation téléphones
    tel_propre = str_replace_all(telephone, "[ .+-]", "")
  ) |>
...
- ✅ **Détecter** des motifs avec `str_detect()` et `str_count()`
- ✅ **Extraire** du texte avec `str_extract()`, `str_sub()`
- ✅ **Remplacer** avec `str_replace()`, `str_remove()`
- ✅ **Transformer la casse** avec `str_to_lower()`, `str_to_upper()`, `str_to_title()`
- ✅ **Nettoyer** avec `str_trim()`, `str_squish()`
- ✅ **Combiner** avec `str_c()`
- ✅ Utiliser des **expressions régulières** de base
...
# ESPACES
str_trim(texte)        # Retirer début/fin
str_squish(texte)      # Retirer début/fin + réduire multiples

# AUTRES
str_length(texte)              # Longueur
str_c(a, b, sep = " ")        # Concaténer
str_split(texte, "sep")       # Découper
